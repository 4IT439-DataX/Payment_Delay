...
## Get rid of irrelevant columns and plots for investigation
data_collection_delay <- subset(data_collection, select = -due_amount) #high correlation due_anount with paid_amount
...
data_collection_delay <- subset(data_collection_delay, select = -due_date) #too corr with payment_date
...
cor.test(data_collection_delay$delay_indiv, data_collection_delay$delay,
         method = "pearson") #corr 0.8083264  - exclude?

#including lagged variables cause of expectation that the current level of the DV is heavily determined by its past level (assumption -> better prediction)
#data_collection_delay <- subset(data_collection_delay, select = -c(delay_indiv,mean_delay_1m,mean_delay_12m,mean_delay_6m,mean_delay_3m))
...
#############################################
### LM compare prediction based on mean variable to real delay
...
#1       0      0 59.19781 54.08611
#1.1     0      0 59.19781 54.08611
...
vip(fit_elnet_reg_ho, bar = FALSE, horizontal = FALSE, size = 1)
