...
# hight correlation with paid_amount
data_collection_delay <- subset(data_collection, select = -due_amount) 
# due_date and payment_date creating delay
...
########### Model - searching for best hyper parameters with HOLDOUT ########### 
...
# get the estimation of the validation error for each couple on the grid
...
for(i in 1:nrow(hyper_grid)){
  for(j in 1:K) {
    ...
    data_validation_fold <- dt_trval[validation_row_indexes, ] # validation fold
