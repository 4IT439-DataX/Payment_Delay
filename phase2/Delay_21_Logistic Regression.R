...
preds = pred.class(preds_probs = preds_probs, cut.off = 0.5)

#accuracy measures
conf_mat = confusionMatrix(data = preds, reference = data_val$delay_21_y, positive = "1")
...
s_logist_roc = roc(response = data_val$delay_21_y, predictor=preds_probs[,2], plot = TRUE, print.auc = TRUE)
