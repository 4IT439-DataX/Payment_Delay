...
First step was to check the correlations of the attributes. As can be seen on Figure \@ref(fig:correlogram), higher positive correlation was discovered between *due_amount* and *paid_amount*, therefore we decided to exclude *due_amount*. We also excluded attribute *due_date*, beacuse delay was calculated as the difference between *due_date* and *payment_date*. *Living_area* was also excluded due to its many factors and low importance.?
...
```
```{r varIm, echo=FALSE, fig.cap="\\label{fig:missing-H}Variable importance for glmnet cv", out.width = '90%'}
