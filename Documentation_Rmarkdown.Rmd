...
Results can be seen on Figure \@ref(fig:corr).

```{r corr, echo=FALSE, fig.cap="Correlation diagram", out.width = '100%'}
knitr::include_graphics("Correlogram.svg")
```

```{r correlogram, eval=FALSE, echo=FALSE, fig.cap="Correlation plot"}
...
#### Basic statistics
Basic statistics computed for numeric variables can be located in Table \@ref(tab:statistics). Distribution of numeric and categorical variables was visualized using boxplots, density plots and histograms, see in Attachments,  Figure \@ref(fig:density) and Figure \@ref(fig:boxplots). NEZOBRAZUJE TAB:

```{r statistics, echo=FALSE, results='asis', tab.cap="Statistics summary.", eval=FALSE}
...
almost same percentage of missing values, as can be seen on the statistics ???REFERENCIA INAK???. 
...
Using a different visualization, that can be seen on Figure\@ref(fig:Missing-complex) or Figure\@ref(fig:missing-H), 
...
```{r Missing-stat, echo=FALSE}
variables_miss <- miss_var_summary(data_collection)
print(variables_miss)
```

```{r Missing-complex, fig.cap="Distribution of missing values.", echo=FALSE}
gg_miss_upset(data_collection)
```

Results can be seen on Figure \@ref(fig:missing-H).

```{r missing-H, echo=FALSE, fig.cap="Missing values distribution", out.width = '100%'}
...
```{r stat_new_delay, echo=FALSE, eval= TRUE, message=FALSE, warning=FALSE, fig.cap="Basic statistics of the added attribute delay."}
delay_plots <- list()
...
Results can be seen on \@ref(fig:stat_new)
```{r stat_new, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Basic statistics of the added attributes.."}
