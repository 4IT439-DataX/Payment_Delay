...
Although the distribution of values across factor variables is not even, we do not think, the findings 
have to be analyzed closely.
All the mentioned findings can be seen on visualizations in Figure \@ref(fig:distribution). 
...
We decided to add new features to create higher-accuracy models. As already have been mentioned, we firstly computed a numeric feature *delay* counting the difference of days between payment_date and due_date. We selected this variable as our target.  

Since we are creating two classification models deciding whether a new payment will be delayed
for more than 21 days or more than 140 days, we created 2 new factor features *delay_21_y* and *delay_140_y*. Variables set 1 if the payment delay is greater than 21 or greater than 140 days.
...
Adding new created attributes, we started to work with 34 variables. We created
2 factor attributes and 8 numerical attributes.
...
It is not surprising, that these attributes have the highest percentage of NAs, since they compute results only every 12/6/3/1 months. We decided to replace the NA values by 0, so they can be later used in the modeling part. We discussed, that this step should not influence the data or the models.   
